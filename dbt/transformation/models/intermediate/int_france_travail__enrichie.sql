...
--
-- Gold layer France Travail : offres enrichies avec géographie et SIRENE.
-- Toutes les colonnes en lowercase_snake_case.
--
-- Jointure SIRENE — priorités décroissantes :
...
--
-- Company name recovery :
--   Si nom absent dans l'offre, cherche si un nom SIRENE apparaît dans la
--   description (géo-filtré + STRPOS). Dédupliqué sur match le plus long.
--
...
-- Candidats SIRENE : toutes les correspondances possibles avec leur priorité
sirene_candidates AS (
    SELECT
        ...
        s.tranche_effectifs,
        CASE
            WHEN o.siret IS NOT NULL AND o.siret = s.siret
                THEN 1  -- SIRET exact
...
final AS (
    SELECT
        ...
        -- Classification métier France Travail
        o.code_rome,
        o.appellation,
        o.experience,
        o.qualification,
